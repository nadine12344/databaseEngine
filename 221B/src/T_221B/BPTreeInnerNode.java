...
public class BPTreeInnerNode<T extends Comparable<T>> extends BPTreeNode<T>  implements Serializable
{
	...
	public PushUp<T> insert(String name,T key, Ref recordReference, BPTreeInnerNode<T> parent, int ptr) throws IOException
	{
		...
		PushUp<T> pushUp = children[index].insert(name,key, recordReference, this, index);
	...
	public Ref getIndexForInsert(T key,int ptr,int maxSize) {
		int index = findIndex(key);
		return children[index].getIndexForInsert(key,index,maxSize);
	}
	/**
	...
	public boolean delete(String name,T key, Ref r,BPTreeInnerNode<T> parent, int ptr) throws IOException 
	{
		...
		for(int i = 0; !done && i < numberOfKeys; ++i)
			if(keys[i].compareTo(key) > 0)
				done = children[i].delete(name,key,r, this, i);
			
		if(!done)
			done = children[numberOfKeys].delete(name,key,r, this, numberOfKeys);
	...
	public Vector<String> searchMoreOrEqual(T key) {
		...
		return children[findIndex(key)].searchMoreOrEqual(key);
	}
	public Vector<String> searchMore(T key) {
		// TODO Auto-generated method stub
		return children[findIndex(key)].searchMore(key);
	}
	/**
